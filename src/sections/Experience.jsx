    ...
    <section id='experience' className='py-32 relative overflow-hidden'>
        ...
        <div className='container mx-auto px-6 relative z-10'>
            ...
            <div className='relative'>
                ...
                w-0.5 bg-linear-to-b from-primary/70 via-primary/30 to-transparent md:-translate-x-1/2 shadow-[0_0_25px_rgba(32,178,166,0.8)]' />
                {/* Experience Items  */}
                <div className='space-y-12'>
                   {experiences.map((exp, index)=>(
                    <div key={index} className='relative grid  md:grid-cols-2 gap-8 animate-fade-in' style={{animationDelay: `${(index +1) * 150}ms`}}>
                      ...
                      {/* Content  */}
                      <div className={`pl-8 md:p-0 ${index % 2 === 0 ? "md:pr-16 md:text-right" : "md:col-start-2 md:pl-16"}`}>
                        <div className={`glass p-6 rounded-2xl border border-primary/30 hover:border-primary/50 transition-all duration-500 `}>
                          ...
                          <div className={`flex flex-wrap gap-2 mt-4 ${index % 2 === 0 ? "md:justify-end" : ""}`}>
                            {exp.technologies.map((tech, techIndex)=>(
                              <span key={techIndex} className='px-3 py-1 bg-surface text-xs rounded-full text-muted-foreground ' key={techIndex}>{tech}</span>
                            ))}
                          </div>
                        </div>
                      </div>
